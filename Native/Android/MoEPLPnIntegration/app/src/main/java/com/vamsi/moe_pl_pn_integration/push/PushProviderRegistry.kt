 ...
 *
 * MoEngage is checked first so that if a payload is ever dual-tagged with both
 * providers' marker keys, MoEngage (the primary platform) wins.
 */
object PushProviderRegistry {

    val default: List<PushProvider> = listOf(
        MoEngagePushProvider(),
     ...
     * refresh and from MoEApplication on app launch so that the token is always
